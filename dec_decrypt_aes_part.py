...
def dec_xor( nibbles, key ):
    ...
    ans = bytearray([ nibbles[i] ^ key[i] for i in range(4) ])
...
def dec_swap_last_two( nibbles ):
    ans = bytearray([ nibbles[0], nibbles[1], nibbles[3], nibbles[2] ])
...
def dec_s_box( nibbles ):
    ...
    ans = bytearray([ s_box[nibble] for nibble in nibbles ])
...
def dec_subtract_last_two_first_two( nibbles ):
    ...
    ans = bytearray([
        (nibbles[0] - nibbles[2]) % 4,
        (nibbles[1] - nibbles[3]) % 4,
        nibbles[2],
        nibbles[3]
    ])
